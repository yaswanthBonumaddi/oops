...
10 బిలియన్ documents. రోజుకి 100 కోట్ల users. ఒకే document ని చాలామంది **ఒకే సమయంలో** సవరించగలగాలి, మరియు ప్రతి ఒక్కరికీ **వెంటనే** కనిపించాలి.

మొదట మొత్తం load ని కొలుద్దాం:

```
10 బిలియన్ docs · రోజుకి 100 కోట్ల users    
...
<b>"user కొట్టిన అక్షరం వెంటనే కనిపించాలా, లేక server ఆమోదం వరకు ఆగొచ్చా?"</b><br><br>
...
ఒక్క user కి ఇది సరిగ్గా పనిచేస్తుంది. ఇద్దరితో?
...
పూర్తి document ని save చేస్తే (last-write-wins) · 2 నిమిషాల session
  ...
  users     | కొట్టిన అక్షరాలు | మిగిలినవి | పోయినవి | నష్టం % | doc "గెంతిన" సార్లు
  ----------+-----------------+----------+---------+--------+-------------------
...
1 user: 16,139 మిగిలాయి. 10 users: 15,924 మిగిలాయి.<br><br>
...
```
ఇంకా తరచుగా save చేస్తే సరిపోతుందా? (3 users    )
...
Client దాన్ని **వెంటనే స్థానికంగా వర్తింపజేస్తుంది** (user ఆగకూడదు), ఆపై server కి పంపుతుంది. Server అందరికీ ప్రసారం చేస్తుంది. అందరూ అన్ని ops ని వర్తింపజేస్తారు.
...
ప్రతి keystroke ని ఒక op గా పంపడం · transform లేకుండా · 60 సెకన్ల session
  ...
  users     | ఆలస్యం | ఒకేలా ముగిసిన sessions | తేడా ఉన్న అక్షరాలు | doc lo %
  ----------+--------+-----------------------+-------------------+---------
          2 |    0ms |             0/60 (0%) |               8.7 |       4%
...
ఒక నిర్దిష్ట session — 2 users, 3 సెకన్లు:

```
  user    A చూస్తున్నది : 9 6 5 10 8 4 0
  user    B చూస్తున్నది : 9 7 6 3 1 0 2
...
మొదట ఇద్దరి దగ్గరా:  A B C D E
  ...
  user    1: స్థానం 1 lo 'X'  →  A X B C D E
  user    2: స్థానం 3 lo 'Y'  →  A B C Y D E     (ఏకకాలంలో)

ఇప్పుడు ops మార్చుకుంటారు:

  user    1 కి ins(3,'Y') వస్తుంది → A X B Y C D E     ← 'Y' C కి *ముందు*
  user    2 కి ins(1,'X') వస్తుంది → A X B C Y D E     ← 'Y' C కి *తర్వాత*
```

<div class="box bad">
<div class="lab">"స్థానం 3" అనేది ఒక సత్యం కాదు — అది ఒక <b>అభిప్రాయం</b></div>
user 2 "స్థానం 3" అన్నప్పుడు వారి ఉద్దేశం "C తర్వాత". కానీ ఆ సందేశం user 1 దగ్గరికి చేరేసరికి, ఆ document lo స్థానం 3 <b>వేరే చోటు</b> — ఎందుకంటే మధ్యలో 'X' వచ్చింది.<br><br>
...
class Client {
  ...
  local(op) {                                   // user    కొట్టినది — వెంటనే కనిపించాలి
    apply(this.doc, op);
...
<b>3.3 &gt; 2</b> — కాబట్టి buffer <b>ఎప్పటికీ ఖాళీ కాదు</b>. అది శాశ్వతంగా పెరుగుతూ ఉంటుంది, మరియు user టైప్ చేసినది ఇతరులకి <b>నిమిషాల</b> ఆలస్యంతో కనిపిస్తుంది.
</div>
...
Log మూడు పనులు చేస్తుంది: **transform కి ఆధారం**, తిరిగి కలిసిన clients కి **catch-up**, మరియు user కి కనిపించే **version history**.
...
users రాసిన మొత్తం text ఒక మోస్తరు cluster lo పడుతుంది. వాళ్ళు <b>ఎలా</b> రాశారో అనే సమాచారం <b>వారానికి</b> అంతకంటే 130 రెట్లు.<br><br>
...
"users ఒక రోజంతా offline lo పనిచేయగలగాలి" అంటే — ఒక్కో చురుకైన doc కి <b>38 MB log</b>.<br><br>
...
user కి కనిపించే "ఈ doc చరిత్ర" అనేది <b>వేరే విషయం</b>, మరియు దానికి per-keystroke ops అవసరం లేదు.<br><br>
...
500 యాదృచ్ఛిక sessions — 2 నుంచి 5 users, 20 నుంచి 420 ms ఆలస్యం, చేర్పులు మరియు తొలగింపులు కలిపి. అయిదు నియమాలు:
...
Cursor ఒక స్థానం — కాబట్టి **దానికీ అదే transform వర్తిస్తుంది**. ఒక remote op వచ్చినప్పుడు, మీ దగ్గర ఉన్న ప్రతి ఇతర user cursor ని ఆ op కి వ్యతిరేకంగా transform చేయాలి (`ins` లాగా, కానీ ఏమీ చేర్చకుండా). ఇది చేయకపోతే cursors టైపింగ్ తో నెమ్మదిగా జారిపోతాయి. Cursors **నిల్వ చేయాల్సిన అవసరం లేదు** — అవి క్షణికమైనవి, మరియు ఒక user వెళ్ళిపోతే అవి పోవాలి.
...
**8. చరిత్ర ఒక నిల్వ సమస్య కాదు — ఒక product నిర్ణయం.** Log document కంటే **134 రెట్లు**, వారానికి **1.5 PB**. దాన్ని కత్తిరించిన క్షణంలో మీరు **"user ఎంతసేపు offline ఉండొచ్చు"** అని నిర్ణయించారు.
...
<b>ప్రతి "instant" UI వెనక ఇదే ఒప్పందం ఉంది. user కి కనిపించే వేగం, engineer కి వచ్చే సంక్లిష్టత.</b>
</div>
